<script lang="ts">
	...
	import CustomerListCard, { type CustomerListItem } from '$lib/components/customers/CustomerListCard.svelte';
	...
	<div class="flex flex-col lg:flex-row gap-6 flex-1 min-h-0">
		...
		<div class="w-full lg:w-1/3 flex flex-col h-full bg-slate-50">
			...
			<div class="flex gap-2 mb-4 shrink-0">
				...
				<button class="inline-flex items-center gap-x-1.5 rounded-lg bg-white px-3 py-2.5 text-sm font-semibold text-slate-700 ring-1 ring-inset ring-slate-200 hover:bg-slate-50 shadow-sm transition-colors shrink-0">
		...
		<div class="w-full lg:w-2/3 h-full flex flex-col bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
			{#if selectedCustomer}
				<!-- Header block -->
				<div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
					<div class="flex items-center gap-5">
						<div class="w-16 h-16 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center text-2xl font-bold">
					...
					<div>
						<button class="inline-flex items-center justify-center rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800 transition-colors shadow-sm gap-2">
				...
				<div class="p-6 flex-1 overflow-y-auto">
					...
					<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
						<div class="rounded-xl border border-slate-200 p-5 flex flex-col items-center justify-center shadow-sm">
							<p class="text-2xl font-bold text-emerald-500 mb-1">฿{(selectedCustomer.lifetime_value ?? 0).toLocaleString()}</p>
					...
					<div class="text-center py-4">
						<p class="text-sm text-slate-500 mb-3">แสดงข้อมูลเบื้องต้น หากต้องการรับข้อมูลทั้งหมดสามารถกดปุ่มนี้</p>
						<button 
